...
function GroupDetailPage() {
  ...
  const fetchGroup = async () => {
    ...
    try {
      ...
      // Only fetch association status if the user is already a member
      if (data.alreadyJoined) {
        const assocData = await getGroupAssociation(user.id, groupId);
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.content}>
            ...
            <div style={styles.diagramCard}>
              ...
              {group.alreadyJoined ? (
                ...
                <>
                  ...
                  <GroupCowDiagram selectedCuts={{}} othersQty={group.othersClaimedQty || {}} />
              ...
              <div style={styles.chatCard}>
                  ...
                  {messages.map((msg) => {
                    ...
                    return (
                      <div
                        ...
                        style={{ ...styles.chatBubbleRow, justifyContent: isMe ? "flex-end" : "flex-start" }}
                      >
                        <div
                          style={{
                            ...styles.chatBubble,
                            ...(isMe ? styles.chatBubbleMe : isSeller ? styles.chatBubbleSeller : styles.chatBubbleOther),
                          }}
                        >
                          {!isMe && (
                            <p style={styles.chatSender}>
                              {msg.senderName}
                              {isSeller && <span style={styles.sellerChatBadge}> SELLER</span>}
                            </p>
                          )}
                ...
                <div style={styles.chatInputRow}>
                  ...
                  <button
                    style={{
                      ...styles.chatSendBtn,
                      ...(!chatInput.trim() || !stompClientRef.current?.connected ? styles.chatSendBtnDisabled : {}),
                    }}
        ...
        {/* ── Bottom: Matching Farms (full width) ── */}
        {farms && (
          <div style={styles.farmsSection}>
            <h2 style={styles.sectionTitle}>Matching Farms</h2>

            {assocError && (
              <div style={{ ...styles.assocBanner, ...styles.assocBannerError }}>{assocError}</div>
            )}
            {association?.status === "PENDING_ASSOCIATION" && (
              <div style={{ ...styles.assocBanner, ...styles.assocBannerPending }}>
                <span>Request sent to <strong>{association.shopName || "seller"}</strong> — waiting for their response.</span>
                {group.isCreator && (
                  <button style={styles.assocCancelBtn} onClick={handleCancelAssociation} disabled={assocLoading}>
                    Withdraw
                  </button>
                )}
              </div>
            ...
            {association?.status === "ASSOCIATED" && (
              <div style={{ ...styles.assocBanner, ...styles.assocBannerActive }}>
                <span>Associated with <strong>{association.shopName || "seller"}</strong></span>
                {group.isCreator && (
                  <button style={styles.assocCancelBtn} onClick={handleRequestDisassociation} disabled={assocLoading}>
            ...
            <div style={styles.distanceFilter}>
              ...
              {distanceFilterEnabled && (
                <div style={styles.sliderContainer}>
                  <label style={styles.sliderLabel}>Show farms within {maxDistance} miles</label>
                  <input
            ...
            <div style={styles.farmsSubheading}>
              ...
              <span style={styles.farmsSub}>
                {filterFarmsByDistance(farms.perfectMatches).length === 0
                  ? distanceFilterEnabled && farms.perfectMatches.length > 0
                    ? "No farms within selected distance."
                ...
                {filterFarmsByDistance(farms.perfectMatches).map((f) => (
                  <div key={f.sellerId} style={{ ...styles.farmCard, borderLeft: "4px solid #2e7d32" }}>
                    <div style={styles.farmHeader}>
                      ...
                      {f.distanceFormatted && <span style={styles.distanceBadge}>📍 {f.distanceFormatted}</span>}
                    </div>
                    ...
                    {group.isCreator && !association && (
                      <button style={styles.assocRequestBtn} onClick={() => handleRequestAssociation(f.sellerId)} disabled={assocLoading}>
                        Request Association
                      </button>
                    )}
                    {association?.status === "ASSOCIATED" && association.sellerId === f.sellerId && (
            ...
            {filterFarmsByDistance(farms.partialMatches).length > 0 && (
              <>
                <div style={{ ...styles.farmsSubheading, marginTop: "24px" }}>
...
const styles = {
  ...
  leaveBtnDisabled: { opacity: 0.5, cursor: "not-allowed" },
  rateBtn: {
    padding: "10px 20px",
    backgroundColor: BUYER_COLOR,
    color: "white",
